...
def get_games(games):
    data = requests.get('https://api-endpoint.igdb.com/games/?search={}&fields=*&limit=50'.format(games), headers=HEADERS).json()
    for i in data:
        print(i['name'])

def get_characters(games):
    data = requests.get('https://api-endpoint.igdb.com/characters/?search={}&fields=*&limit=50'.format(games), headers=HEADERS).json()
    for i in data:
        print(i['name'])
